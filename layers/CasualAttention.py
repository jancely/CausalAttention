...
class CasualAttention(nn.Module):
    ...
    def casual(self, corr, order):
        ...
        for i in range(1, length):
            ...
            w_sequence.append(w_j)

        frac_array = torch.sqrt(torch.tensor(w_sequence, device=corr.device, dtype=torch.float32))
        frac_array = frac_array.view(corr.shape[-2], corr.shape[-1]).unsqueeze(0).unsqueeze(0)

        casual_corr = corr * frac_array
    ...
    def forward(self, queries, keys, values, attn_mask, tau=None, delta=None):
        ...
        corr = torch.fft.irfftn(res, s=(H, D), dim=(-2, -1))

        corr = self.casual(corr, self.order)
